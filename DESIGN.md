...
The first public homepage now adopts the reference's white editorial canvas, bold dark type, outlined actions, and prominent creative imagery. Its hero uses a custom wide composition derived from user-provided book-cover, business-card, menu, signage, and merchandise work. The foreground products share one believable studio counter, the sign is mounted on the background wall, and the left side remains quiet for live copy over the full-bleed image; mobile uses a focused crop beneath the copy. The concept-study section remains explicitly illustrative rather than presented as client work. The shipped stylesheet is `public/assets/css/app.css`, which imports a public copy of the saved Primo token file at `public/assets/css/tokens.css`. Keep that copy synchronized with `design-system/tokens.css` when tokens change.
