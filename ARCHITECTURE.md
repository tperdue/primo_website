...
Requests marked Ready to quote or Closed, Quote Created can be converted once into an actual commercial quote. Conversion finds or creates a lightweight customer by normalized email, links matching request history, and creates a customer/contact snapshot on the quote plus editable line items, calculated subtotal/discount/tax/total/deposit values, a status-history entry, and an immutable JSON revision snapshot. The request then remains Closed, Quote Created while the quote owns its separate draft, delivery, acceptance, and decline lifecycle. Customer administration provides protected search, contact/address editing, internal notes, and a chronological request/quote/status timeline. Profile edits do not rewrite quote snapshots, and customer deletion is intentionally unavailable while retention and archival policy remain undefined. Quote math is recalculated server-side on every save. Customer-facing proposals use a random 256-bit bearer token, are available only in public quote states, and return private no-store/noindex responses. Delivery uses the configured email service and records its outcome without falsely marking an undelivered quote as sent.

Customers can accept or decline a ready or sent quote from its secure proposal. The POST action requires CSRF, rejects expired quotes, permits only one final customer decision, records the customer/system actor and response timestamp in immutable quote history, and creates a new quote revision. The bearer token remains stable so the final proposal stays available as a read-only commercial record. Accepted quotes can be converted once into an admin-only lightweight project with a stable project number, customer and quote links, schedule, operational status, private notes, and a future customer-visible update.
