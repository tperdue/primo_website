...
<main class="admin-main admin-main-wide">
    ...
    <div class="admin-customer-summary"><div><span>Quote requests</span><strong><?= count($requests) ?></strong></div><div><span>Quotes</span><strong><?= count($quotes) ?></strong></div><div><span>Accepted quotes</span><strong><?= (int) $acceptedCount ?></strong></div></div>
    <div class="admin-customer-layout">
        ...
        <section class="admin-customer-history" aria-labelledby="relationship-history-heading"><div class="admin-section-heading"><div><h2 id="relationship-history-heading">Relationship history</h2><p>Requests and quotes are ordered by when they entered the workspace.</p></div></div>
            <?php if ($timeline === []): ?><div class="admin-empty"><h3>No relationship activity</h3><p>Linked quote requests and quotes will appear here.</p></div><?php else: ?><div class="admin-timeline"><?php foreach ($timeline as $entry): ?><article><div class="admin-timeline-marker" aria-hidden="true"></div><div class="admin-timeline-content"><div><span><?= esc(strtoupper($entry['type'])) ?> &middot; <?= esc(substr($entry['date'], 0, 16)) ?></span><span class="admin-status admin-status-<?= esc(str_replace('_', '-', $entry['status']), 'attr') ?>"><?= esc($entry['statusLabel']) ?></span></div><h3><a href="<?= esc($entry['url'], 'attr') ?>"><?= esc($entry['title']) ?></a></h3><p><?= esc($entry['summary']) ?></p></div></article><?php endforeach ?></div><?php endif ?>
        </section>
    </div>
</main>
