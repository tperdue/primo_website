...
<main class="admin-main admin-main-wide">
    ...
    <?php if ($customers === []): ?>
        <div class="admin-empty"><h2><?= $query === '' ? 'No customers yet' : 'No matching customers' ?></h2><p><?= $query === '' ? 'Customer profiles appear here when you add one or create a quote from a request.' : 'Try a broader name, email, business, or phone search.' ?></p><?php if ($query === ''): ?><a href="/admin/customers/new">Create a customer &nearr;</a><?php endif ?></div>
    <?php else: ?>
