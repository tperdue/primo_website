...
- Customer proposal decisions are POST-only and CSRF-protected. Only ready or sent quotes that have not expired can be accepted or declined; the first final decision wins, records a bounded optional note plus customer/system audit source, and leaves the bearer link read-only. Raw visitor IP addresses are not stored.
